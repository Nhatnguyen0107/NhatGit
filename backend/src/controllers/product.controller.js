...
class ProductController {
    ...
    async getProductReviews(req, res) {
        try {
            ...
            // Import ReviewService here to avoid circular dependency
            ...
            // Format response as specified in document
            const response = {
                averageRating: result.stats.avgRating,
                totalReviews: result.stats.totalReviews,
                ratingCounts: result.stats.ratingCounts,
                reviews: result.reviews.map(review => ({
                    id: review.id,
                    user: review.customer?.user?.username || 'Anonymous',
                    rating: review.rating,
                    comment: review.comment,
                    created_at: new Date(review.created_at).toISOString().split('T')[0],
                    is_verified_purchase: review.is_verified_purchase,
                    avatar: review.customer?.user?.avatar
                }))
            };
